...
public class Hotel implements Serializable {
    ...
    /**
     * Verifies if a veterinary with the given ID exists in the veterinary list.
     *
     * @param vetId The ID of the veterinary to check.
     * @return `true` if a veterinary with the specified ID exists; `false` otherwise.
     * @throws UnknownKeyException if the veterinary ID does not correspond to any veterinary.
     */
    public boolean verifyVeterinary(String vetId) throws UnknownKeyException {
        for (Veterinary aux : _veterinarysList) {
            if (aux.getEmployeeId().equals(vetId)) {
                return true;
            }
        }
        throw new UnknownKeyException("Id doesn't correspond to a Veterinary.");
    }
    ...
    public void verifyHabitat(String habitatId) throws UnknownKeyException {
        boolean found = false; //FiXME Nem deus sabe explicar o que é isto
        for (Habitat habitat : _habitatsList) {
            if (habitat.getHabitatId().equalsIgnoreCase(habitatId)) {
                found = true;
                break;
    ...
    public List<Animals> returnAnimaisInHabitat(String habitatId) throws CoreUnknownHabitatKey {
        ...
        for (Animals animal : _animalsList) {
            ...
            if (animal.getAnimalHabitat().equalsIgnoreCase(habitatId)) {
                animalsInHabitat.add(animal); // Add the animal to the list
            }
        }
        return animalsInHabitat; // Return the list of animals in the habitat
    }
